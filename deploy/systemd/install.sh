...
configure_nginx() {
    ...
    # Generate nginx configuration with HTTP enabled and HTTPS commented out
    cat > "${nginx_conf}" <<EOF
...
# 2. Uncomment the HTTPS server block below
# 3. Update certificate paths
# 4. Optionally uncomment the HTTP->HTTPS redirect
...
# =============================================================================
# To enable HTTPS:
# 1. Install certificates (e.g., certbot --nginx -d ${DOMAIN_NAME})
# 2. Uncomment the server block below
# 3. Update ssl_certificate and ssl_certificate_key paths
# 4. Comment out or redirect the HTTP location blocks above
# =============================================================================
...
EOF
    ...
    if nginx -t; then
        ...
        log_success "Nginx configured (HTTP enabled, HTTPS commented out)"
    else
...
main() {
    ...
    echo "  3. To enable HTTPS, edit /etc/nginx/sites-available/pemly"
    echo "     and uncomment the SSL server block"
    echo
}
