...
write_cert_config() {
    local target="${SCRIPT_DIR}/traefik/dynamic/certificates.yml"
    cat > "${target}" <<'EOF'
# Generated by bootstrap.sh when a certificate was installed. Safe to delete:
# removing it returns Traefik to its built-in self-signed certificate.
tls:
  certificates:
